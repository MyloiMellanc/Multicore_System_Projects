...
void init_thread() {
	int core_num = _s->core_num - 1;
	thread_num = core_num;
	thread_range_list = (thread_range*)malloc(thread_num * sizeof(thread_range));

	//int range_start = 0;
	int range_end = _s->map_size - 1;	//Because, Range start is zero.

	float range_per_thread = ((float)_s->map_size / (float)thread_num);
	float x_position = 0;
	...
	//for last one
	thread_range_list[thread_num - 1].x_start = (int)x_position;
	thread_range_list[thread_num - 1].x_end = range_end;
...
//It is not dangerous in this project, because OpenMP use use this function with critical section preprocessor
void toggle_thread_counter() {
	thread_toggle_counter++;
...
void MP_move_devils_and_spread_plague() {
	...
	//toggle and wait for pass trigger, with critical section
...
int get_devil_count() {
	...
	for (int i = 0; i < number; i++) {
		for (int j = 0; j < number; j++) {
			for (int k = 0; k < number; k++) {
				count += devil_cubic[i * number * number + j * number + k].on_devil;
			}
		}
	}
	...
	for (int x = min_x; x <= max_x; x++) {
		for (int y = min_y; y <= max_y; y++) {
			for (int z = min_z; z <= max_z; z++) {
					count += devil_cubic[x * number * number + y * number + z].on_devil;
			}
		}
	}
...
void devil_live_plague_stage(struct setup *s) {
	...
	//move all devils.
	//USING OpenMP
    int thread_num = _s->core_num;
    #pragma omp parallel num_threads(thread_num)
    {
        int this_thread = omp_get_thread_num();
        if (this_thread != thread_num - 1)
            MP_move_devils_and_spread_plague();
...
void live_dead_stage(struct setup *s) {
    //do nothing. already done in devil_live_plague stage.
